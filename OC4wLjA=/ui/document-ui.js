(function(){
	flobn.ui.internet = flobn.widgets.Page.extend({
		...
		listeners: {
			'.pagination a click': function(e){
				...
				this.paginate(e.target.href+'&app='+app+'&rp='+rp, 'document');
				e.preventDefault();
			},
			'.pagination input keydown': function(e){
				...
				this.paginate(srcTarget.data('page')+'&app='+app+'&rp='+rp+'&offset='+offset,'document');
				e.preventDefault();				
			},
			'.pagination select change': function(e){
				...
				this.paginate(srcTarget.data('page')+'&app='+app+'&rp='+rp,'document');
				e.preventDefault();				
			},
		...
		saveChildSlider: function(el, newVal, oldVal){
			...
			$('#export-csv').attr('href','index.php?pag=document&act=reports-document&app='+o.app);
			$('#export-pdf').attr('href','index.php?pag=document&render=pdf&app='+o.app);
		...
		refreshChart: function(type){
			...
			switch(type){
				case 'productivity'	:				
					$("#productivity_chart .chart").load("index_ajax.php?pag=chart_documentproductivity");
					break;
				case 'category':
					// $("#categories_chart .chart").load("index_ajax.php?pag=chart_documentmonitoring");
					break;				
			}
		},
		showNotification: function(e,type){
			...
			var msg = '', title = '', func = function(){
				...
				window.location.href = ['index.php?pag=document&f',department].join('=');
			};
			switch(type){
				case 1:
	...
	$(function(){
		flobn.register('document',new flobn.ui.internet('#document'));			   
	});
})();