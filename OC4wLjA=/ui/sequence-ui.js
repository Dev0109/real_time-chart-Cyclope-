(function(){
	flobn.ui.sequence = flobn.widgets.Page.extend({
		init: function(id){
			this._super(id);
			this.tie({
			shortcutToday: true,
		...
		addFilter: function(param){
			...
			$('#export-csv').attr('href','index.php?pag=sequence&act=reports-sequence&app='+o.app+'&win='+o.win+'&wact='+o.wact);
			$('#export-pdf').attr('href','index.php?pag=sequence&render=pdf&app='+o.app+'&win='+o.win+'&wact='+o.wact);
		...
		showNotification: function(e,type){
			...
			var msg = '', title = '', func = function(){
				...
				window.location.href = ['index.php?pag=sequence&f',department].join('=');
			};
			switch(type){
				case 1:
	...
	$(function(){
		flobn.register('sequence',new flobn.ui.sequence('#sequence'));			   
	});
})();