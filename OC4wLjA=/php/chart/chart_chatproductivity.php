...
$application = $dbu->query("SELECT SUM(session_application.duration) AS duration,
						...
						AND application.application_type = 1
						".$app_filter."
...
$prodstack['animationEnabled'] = true;
...
while ($productivity->next()){
	if($productivity->f('productive') == 3){
		...
		$children = $dbu->query("SELECT SUM(session_chat.duration) AS app_duration,
									application_productivity.productive AS productive
	    			 FROM session_chat
					 INNER JOIN session ON session.session_id = session_chat.session_id
					".$app_join."
					LEFT JOIN application_productivity ON application_productivity.department_id = member.department_id 
								  							AND application_productivity.link_id = session_chat.chat_id
								  							AND application_productivity.link_type = 1  							
					WHERE session_chat.time_type = 0 AND 5=5 
					AND application_productivity.productive >= 0
					AND session_chat.application_id = '".$productivity->f('application_id')."'
					".$app_filter."
...
$tempchart['green'] = $chart['green'];
unset($chart['green']);
$tempchart['red'] = $chart['red'];
unset($chart['red']);
$tempchart['rest'] = $chart['rest'];
unset($chart['rest']);

$total = $chart['total'];
unset($chart['total']);
$i = 0;
foreach ($tempchart as $key => $val){
	// if ((($val * 100) / $total) > 1) {
		// $theminus = 0.01;
	// } else {
		$theminus = 0;
...
$prodstack['data[$i]']['legendText'] = $labels[$key] . ' (' . number_format((($val * 100) / $total) - $theminus,2) . '%)';
