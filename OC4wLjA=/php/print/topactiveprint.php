...
$fts->define(array('main' => "topactiveprint.html"));
...
$dbu->query("SELECT SUM(session_activity.duration) AS active_time,member.member_id,member.logon FROM session_activity 
INNER JOIN session ON session.session_id = session_activity.session_id ".$total_join."
WHERE 1=1 ".$total_filter." AND session_activity.activity_type = 1 GROUP by session.member_id ORDER BY active_time DESC LIMIT 15");
...
while ($dbu->move_next()){	
	...
	$fts->assign(array(
		...
		'ACTIVE_TIME_H' => intval(intval($dbu->f('active_time')) / 3600),
		'ACTIVE_TIME_M' => (intval($dbu->f('active_time')) / 60) % 60,
		'ACTIVE_TIME_S' => intval($dbu->f('active_time')) % 60,
	));
