...
$fts->define(array('main' => "topproductiveprint.html"));
...
while ($productivity->next()){
	...
	if($productivity->f('productive') == 3){
		...
		$children = $dbu->query("SELECT SUM(session_log.duration) AS app_duration,
							...
							AND session_log.active = 1
							AND 2=2
							AND application_productivity.productive = 2
...
foreach ($durations as $member_id => $duration){
	...
	$fts->assign(array(
		...
		'PRODUCTIVE_TIME_H' => intval(intval($tags['duration']) / 3600),
		'PRODUCTIVE_TIME_M' => (intval($tags['duration']) / 60) % 60,
		'PRODUCTIVE_TIME_S' => intval($tags['duration']) % 60,
	));
