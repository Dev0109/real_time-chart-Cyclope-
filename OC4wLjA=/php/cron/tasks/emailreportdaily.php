...
debug_log("================================================",'log-emails-report-daily');
debug_log("report send selected mail options",'log-emails-report-daily');
debug_log("report send selected mail options Username ".$mail->Username,'log-emails-report-daily');
debug_log("report send selected mail options From ".$mail->From,'log-emails-report-daily');
debug_log("report send selected mail options Sender ".$mail->Sender,'log-emails-report-daily');
debug_log("report send selected mail options Password ".$mail->Password,'log-emails-report-daily');
debug_log("report send selected mail options Host ".$mail->Host,'log-emails-report-daily');
debug_log("report send selected mail options Port ".$mail->Port,'log-emails-report-daily');
debug_log("report send selected mail options Mailer ".$mail->Mailer,'log-emails-report-daily');
debug_log("report send selected mail options SMTPSecure ".$mail->SMTPSecure,'log-emails-report-daily');
debug_log("report send selected mail options SMTPAuth ".$mail->SMTPAuth,'log-emails-report-daily');
...
while($dbu->move_next())
{
	if(!$_REQUEST['single'] || $_REQUEST['single'] == $dbu->f('email_report_id')){
		...
		{
			debug_log("report group ".$auxiliary_dbu->f('name'),'log-emails-report-daily');
			$group = array();
		...
		while ($auxiliary_dbu->move_next())
		{
			debug_log("report type ".$l->lookup($opt[$auxiliary_dbu->f('type')]),'log-emails-report-daily');
		...
		while ($auxiliary_dbu->move_next())
		{
			debug_log("report receiver ".$auxiliary_dbu->f('email'),'log-emails-report-daily');
			array_push($report['receivers'],$auxiliary_dbu->f('email'));
		}
		array_push($reports,$report);
		debug_log("report description ".$report['description'],'log-emails-report-daily');
		debug_log("report name ".$report['name'],'log-emails-report-daily');
		debug_log("report id ".$report['email_report_id'],'log-emails-report-daily');
		debug_log("report sender ".$report['sender'],'log-emails-report-daily');
	}
}
$frequency = $l->lookup("Daily");
foreach ($reports as $report)
{
	...
	$description = $report['description'];
 	...
 	{
		debug_log("report receiver ".$receiver,'log-emails-report-daily');
	...
	foreach ($report['types'] as $type)
 	{
		debug_log("report type ".$l->lookup($opt[$type]),'log-emails-report-daily');
 		$selected_reports .= $l->lookup($opt[$type]).', ';
 		$for = '';
 		foreach ($report['groups'] as $group)
 		{
			debug_log("report group ".$group['name'],'log-emails-report-daily');
 			$for .= $group['name'].', ';
 			session_start();
			...
			switch ($report['attachment_type']){
    			case '1': $_SESSION['attachment_name'] = str_replace(' ','_',text_process($l->lookup($opt[$type]).' '.$l->lookup('Daily').' (' . $timefilter . ') ' .$group['name']).'.pdf');
    				break;
    			case '2': $_SESSION['attachment_name'] = str_replace(' ','_',text_process($l->lookup($opt[$type]).' '.$l->lookup('Daily').' (' . $timefilter . ') ' .$group['name']).'.csv'); 
    				break;
    		...
    		session_write_close();
			$timer = time();
			debug_log("report attachment include at " . $timer . ": " . $_SESSION['attachment_name'],'log-emails-report-daily');
    		switch ($report['attachment_type']){
	    		case '1': do_post_request($site_url.'/index.php','pag='.$target[$type].'&render=pdf&send=email&session_id='.session_id());
				debug_log("report attachment url: " . $site_url.'/index.php?pag='.$target[$type].'&render=pdf&send=email&session_id='.session_id(),'log-emails-report-daily');
	    				 break;
	    		case '2': do_post_request($site_url.'/index.php','pag='.$page[$type].'&act=reports-'.$targetcsv[$type].'&session_id='.session_id()); 
				debug_log("report attachment url: " . $site_url.'/index.php?pag='.$page[$type].'&act=reports-'.$targetcsv[$type].'&session_id='.session_id(),'log-emails-report-daily');
	    				 break;
	 
    		}
 			if (file_exists($tmp."/".$_SESSION['attachment_name']))
 			{
				debug_log("report attachment included at " . time() . ", diff of " . (time() - $timer) . ": " . $_SESSION['attachment_name'],'log-emails-report-daily');
				debug_log("memory usage at " . memory_get_usage()/1024.0 . " kb",'log-emails-report-daily');
	...
	$mail->Body=$body;
	debug_log("report send attachment ",'log-emails-report-daily');
	$mail->Send();
	debug_log("report sent error for DAILY (if empty, means it is ok): ".$mail->ErrorInfo,'log-emails-report-daily');
