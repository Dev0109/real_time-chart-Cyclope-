(function () {
	...
	var themes =
	{
		"theme1": {
			...
			Title: {
				fontFamily: isCanvasSupported ? "Arial, sans-serif" : "arial",
				fontSize: 33,
				fontColor: "#3A3A3A",
				fontWeight: "bold",
				verticalAlign: "top",
			...
			Axis: {
				...
				titleFontFamily: isCanvasSupported ? "Arial, sans-serif" : "arial",
				//titleFontWeight: "bold",

				//labelFontFamily: "Times New Roman, Times, serif",
				labelFontFamily: isCanvasSupported ? "Arial, sans-serif" : "arial",
	...
	Chart.prototype._initialize = function () {
		...
		// if (this._options.data) {
			console.log("start==>", this._options);
			console.log("start123==>", this._options.data);
	...
	Chart.prototype.renderColumn = function (plotUnit) {
		...
		if (typeof (this._options.barwidth) !== "undefined") {
			this.barwidth = this._options.barwidth;
		} else {
			this.barwidth = (((plotArea.width / Math.abs(plotUnit.axisX.maximum - plotUnit.axisX.minimum)) * Math.abs(xMinDiff)) / plotUnit.plotType.plotUnits.length * .9) << 0
		}
	...
	Chart.prototype.renderStackedArea100 = function (plotUnit) {
		...
		if (typeof (this._options.barwidth) !== "undefined") {
			...
			this.barwidth = (((plotArea.width / Math.abs(plotUnit.axisX.maximum - plotUnit.axisX.minimum)) * Math.abs(xMinDiff)) * .9) << 0
		}
		var barWidth = this.barwidth;
	...
	Chart.prototype.renderBubble = function (plotUnit) {
		...
		if (typeof (this._options.barwidth) !== "undefined") {
			...
			this.barwidth = (((plotArea.width / Math.abs(plotUnit.axisX.maximum - plotUnit.axisX.minimum)) * Math.abs(xMinDiff)) / totalDataSeries * .9) << 0;
		}
		var barWidth = this.barwidth;
	...
	Chart.prototype.renderCandlestick = function (plotUnit) {
		...
		if (typeof (this._options.barwidth) !== "undefined") {
			...
			this.barwidth = (((plotArea.width / Math.abs(plotUnit.axisX.maximum - plotUnit.axisX.minimum)) * Math.abs(xMinDiff)) * .7) << 0;
		}
		var barWidth = this.barwidth;
	...
	ToolTip.prototype._initialize = function () {
		...
		if (this.enabled) {
			...
			toolTipHtml += "padding: 5px 20px;";
			toolTipHtml += "font-family: 'Avinira-regular';";
			toolTipHtml += "font-weight: 400;";
			// toolTipHtml += "font-style: " + (isCanvasSupported ? "italic;" : "normal;");
			toolTipHtml += "font-size: 12px;";
			toolTipHtml += "color: #fff;";
			...
			toolTipHtml += isCanvasSupported ? "background: #343f50;" : "background:#343f50;";
