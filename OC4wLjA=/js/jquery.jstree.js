		...
		_fn : {
			...
			change_state : function (obj, state) {
				...
				if(state){ obj.find("li").andSelf().removeClass("jstree-checked jstree-undetermined").addClass("jstree-unchecked");	}
					...
					obj.parentsUntil(".jstree", "li").addClass('jstree-undetermined');
...
//*/
/* 
 * jsTree select plugin 1.0
 * Renders the tree inside a select element,works with the checkbox plugin (still a bit experimental)
 * NOTE: does not check language versions 
 */
(function ($) {
var multiselectID = 0;
	$.jstree.plugin("select", {
		__init : function () {		
			var el = this.get_container().find('select'),
				o = this._get_settings().select,
				html = [],				
				title = el.attr('title'),
				id = el.attr('id') || multiselectID++; // unique ID for the label & option tags
				this._isOpen = false;
				//create the button 1st
				...
				//apend it
				this.get_container().before(html.join(''));
				this.button = this.get_container().prev();
				this.buttonLabel = this.button.find('span').eq(-1);
				
				//now get all the data and create the tree
				//first we check to see if we have a root to create				
				var ul = $('<ul>');
				if(o.root){
					var ul = this._parse_json(o.root);
					ul.find('> li').append('<ul>');
					var d = ul.find('ul');
				}else{
					var d = ul;
				}				
				this.get_container().addClass('ui-multiselect-menu ui-widget ui-widget-content ui-corner-all');
				
				el.find(' > optgroup').each(function(){
					var $this = $(this);
					var label = $this.attr('label');
					var attrs = $.isArray(o.attrs) ? o.attrs : [ ];
					var $li = $('<li>');
					$.each(attrs,function(i,v){						
						$li.attr(v,$this.attr(v));
					});
					$li.data('value',$this.attr('value'));										
					$li.append('<a href="#">'+label+'</a>');					
						...
						$this.children().each(function(){
							...
							var $$li = $('<li>');							
								...
								if($this.attr('selected')){
									$$li.addClass('jstree-checked');
								}									
								$$li.data('value',$this.attr('value'));
								$$li.data('index', (o.root ? this.index+1 : this.index));
				...
				//if there is a root we need to add it
				if(o.root){					
					el.find('> :first-child').before('<option value="">'+o.root.data+'</option>');
				}
				if(!o.multiple){
					this.get_container().addClass('jstree-radio');
				}
				//move the select out of the way
				this.selectEl = this.get_container().after(el).append(ul).next().hide();
		...
		_fn : { 
			_bindEvents: function(){
				var self = this, button = this.button;
					...
					function clickHandler( e ){
						self[ self.button.hasClass('ui-state-active') ? 'close' : 'open' ]();
						return false;
					}
				...
				this.get_container().closest('form').bind('reset', function(){
					setTimeout(function(){ self.update(); }, 1);
				});
			},
			// set button width
			_setButtonWidth: function(){
				var width = this.get_container().outerWidth(),
					o = this._get_settings().select;
			...
			open: function(e){			
				var self = this,
					button = this.button,
					menu = this.get_container(),
					speed = this.speed,
					o = this._get_settings().select;

				// bail if the multiselectopen event returns false, this widget is disabled, or is already open 
				if( button.hasClass('ui-state-disabled') || this._isOpen ){
					return;
				}
				...
				menu.scrollTop(0).height(o.height);
					menu.css({ 
			...
			// close the menu
			close: function(){
				var self = this, o = this._get_settings().select, effect = o.hide, speed = this.speed;
				...
				this.get_container().hide(effect, speed);
				this.button.removeClass('ui-state-active');
			},
			update:function(){
				var o = this._get_settings().select,
					numChecked = this.get_container().find('li.jstree-leaf.jstree-checked').length;
					
				if( numChecked === 0 ){
					value = o.noneSelectedText;
				} else {
					if( o.selectedList && o.selectedList > 0 && numChecked <= o.selectedList){
						value = this.get_container().find('li.jstree-leaf.jstree-checked > a').map(function(){ 
								return this.lastChild.nodeValue; 
							}).get().join(', ');
					} else {
			...
			change_state : function (obj, state) {
				...
				if(!this.get_settings().select.multiple){
					if(obj.find('> ul').length){
						return;//no soup for you!;
					}
					this.get_container().find('.jstree-checked,.jstree-undetermined').removeClass('jstree-checked').addClass('jstree-unchecked');
					this.close();
				}
				...
				var selectedValues = [];
				this.get_container().find('.jstree-leaf.jstree-checked').each(function(){
					selectedValues.push($(this).data('value'));
				});				
				this.selectEl.val(selectedValues);
				this.update();
				this.__callback(obj);
			},			
			check_node : function (obj) {
			...
			_repair_state : function (obj) {
				...
				if(c === 0) { if(obj.hasClass("jstree-undetermined")) { this.check_node(obj); } }
					...
					obj.parentsUntil(".jstree","li").removeClass("jstree-checked jstree-unchecked").each(function(){
						var $this = $(this),
							child = $this.find('li').length;
							sel = $this.find('.jstree-checked').length;
							if(sel == child){
		...
		_fn : {
			...
			change_state : function (obj, state) {
				...
				if(state) { 
					...
					if(this.data.ui) { this.data.ui.last_selected = obj; }
					this.data.checkbox2.last_selected = obj;
				}
				// obj.parentsUntil(".jstree", "li").each(function () {
					// var $this = $(this);
					// if(state) {
