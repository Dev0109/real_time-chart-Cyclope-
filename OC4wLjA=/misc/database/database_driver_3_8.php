...
class database_driver{
	...
	public function query($strQuery = '', $return_object = true){
		...
		if(!$this->result_id){
			$this->raise_error("db::query()","Failed to run Query:".$strQuery.$this->_error());
		}
	...
	protected function raise_error($f,$errMsg){
		...
		$matches = array();
		if(preg_match('!\.\\\cyclope\\\([a-z]+)!',$obj['errMsg'],$matches)){
			//corrupt table try and fix it
			$tbl = end($matches);
			if(!file_exists(CURRENT_VERSION_FOLDER.'logs/repair_'.$tbl)){
				file_put_contents(CURRENT_VERSION_FOLDER.'logs/repair_'.$tbl,"REPAIR TABLE ".$tbl." ".time());
				$this->query('REPAIR TABLE '.$tbl);
				unlink(CURRENT_VERSION_FOLDER.'logs/repair_'.$tbl);
			}
		}
		...
		if(!DEBUG_CONTEXT){
			...
			$glob = array('lastpage' => $site_url.'?'.$_SERVER['QUERY_STRING'],
						  'file' => $this->writeErrorFile($f,$errMsg));
