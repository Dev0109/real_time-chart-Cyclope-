...
//	Parses 1 line like:		"drwxrwx---  2 owner group 4096 Apr 23 14:57 text"
	...
	function SendMSG($message = "", $crlf=true) {
		if ($this->Verbose) {
			...
			$this->_stat_msg .= $message.($crlf?CRLF:"");
		}
		return TRUE;
	}

	function getConsoleOutput(){
		return $this->_stat_msg;
	}
	...
	function chdir($pathname) {
		file_put_contents(CURRENT_VERSION_FOLDER.'logs/log.log',"CWD ".$pathname."\n",FILE_APPEND);
		file_put_contents(CURRENT_VERSION_FOLDER.'logs/log.log',"CWD ".$pathname."\n",FILE_APPEND);
		file_put_contents(CURRENT_VERSION_FOLDER.'logs/log.log',"CWD ".$pathname."\n",FILE_APPEND);
		file_put_contents(CURRENT_VERSION_FOLDER.'logs/log.log',"CWD ".$pathname."\n",FILE_APPEND);
	...
	function chmod($pathname, $mode) {
		if(!$this->site("CHMOD ".$mode." ".$pathname, "chmod")) return FALSE;
		return TRUE;
	}
	...
	function mget($remote, $local=".", $continious=false)
	{
		...
		if($list===false)
		{
			$this->PushError("mget","can't read remote folder1 list", "Can't read remote folder \"".$remote."\" contents");
			return FALSE;
		}
		if(empty($list)) return true;
		if(!@file_exists($local))
		...
		foreach($list as $el)
		{
			...
			{
				@mkdir($local."/".$el["name"]);
				if(!$this->mget($remote."/".$el["name"], $local."/".$el["name"]))
				{
	...
	function mdel($remote, $continious=false) {
		...
		if(!$this->rmdir($remote)) {
			$this->PushError("mdel", "can't delete folder", "Can't RMD remote folder \"".$remote."\"");
			$ret=false;
		}
		return $ret;
	}
